...
def run_n_times(repetitions, block):
    total_units = 0
...
def comparison_or_chain(size, repetitions):
    sink = [False]
    ...
    def block():
        ...
        for i in range(size):
            value = (i % 10) + 1
            result = (value == 1 or value == 2 or value == 3
                      or value == 4 or value == 5)
        sink[0] = result
        return size

    return run_n_times(repetitions, block)


def comparison_in_tuple(size, repetitions):
    sink = [False]
...
def main():
    ...
    elif args.mode == "comparison-or-chain":
        total_units = comparison_or_chain(args.size, args.repetitions)
    elif args.mode == "comparison-in-tuple":
        total_units = comparison_in_tuple(args.size, args.repetitions)
